...
void hash_table_v1_add_entry(struct hash_table_v1 *hash_table,
							 ...
							 uint32_t value)
{
	int err = pthread_mutex_lock(&foo_mutex); // lock returns 0 if successful
	// critical section
	if (err != 0)
	{
		exit(1);
	...
	if (list_entry != NULL)
	{
		...
		err = pthread_mutex_unlock(&foo_mutex);
		// end of critical section in the if statement
		if (err != 0)
		{
			exit(1);
		}
	...
	// end of critical section if list_entry == NULL (if the list is empty)
	err = pthread_mutex_unlock(&foo_mutex); // unlock also returns 0 if successful
	if (err != 0)
	{
		exit(1);
	}
